...
The page includes the Visto logo system under `public/assets/brand-kit` and keeps HAWKS BI as the quiet maker mark. It does not import code, routes, styles, or dependencies from the Visto application. The commercial layer uses GSAP for the staged hero, scroll reveals, progress fill, and pointer-driven 3D tilt; the product scene itself is CSS 3D so it stays lightweight.
